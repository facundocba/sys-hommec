            ...
            <div id="frecuencia-horas-section" style="display: none;">
                    ...
                    <select class="form-select" id="horas_por_dia" name="horas_por_dia">
                        ...
                        <?php foreach ([1, 2, 3, 4, 6, 8, 12, 24] as $opt): ?>
                            <option value="<?php echo $opt; ?>"
                                <?php echo (($formData['horas_por_dia'] ?? '') == $opt) ? 'selected' : ''; ?>>
                                <?php echo $opt; ?> hs/día
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <input type="number" step="0.5" min="0" class="form-control" id="horas_mes" name="horas_mes"
                           value="<?php echo htmlspecialchars($formData['horas_mes'] ?? ''); ?>"
                           placeholder="Autocalculado">
                    <input type="hidden" id="horas_mes_override" name="horas_mes_override"
                           value="<?php echo !empty($formData['horas_mes_override']) ? '1' : '0'; ?>">
                    <small class="text-muted" id="horas_mes_hint">Se calcula automáticamente al elegir hs/día.</small>
                    <button type="button" class="btn btn-link btn-sm p-0 mt-1" id="horas_mes_reset" style="display:none;">
...
<script>
    ...
    function showModoSesiones() {
        ...
        if (horasPorDiaSelect) {
            horasPorDiaSelect.removeAttribute('required');
            horasPorDiaSelect.value = '';
            horasMesInput.value = '';
    ...
    function showModoHoras() {
        ...
        if (horasPorDiaSelect) {
            horasPorDiaSelect.setAttribute('required', 'required');
        }
