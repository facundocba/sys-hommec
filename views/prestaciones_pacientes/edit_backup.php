...
<form method="POST" action="<?php echo baseUrl('prestaciones-pacientes/store/' . $patient['id']); ?>">
    ...
    <div class="form-section">
        <div class="form-section-header">
            ...
            <h2 class="form-section-title">Servicio y Profesional</h2>
        </div>

        <div class="row g-3">
            <div class="col-md-6">
                <label for="id_prestacion" class="form-label">Servicio <span class="text-danger">*</span></label>
                <select class="form-select <?php echo isset($formErrors['id_prestacion']) ? 'is-invalid' : ''; ?>"
                        id="id_prestacion" name="id_prestacion" required>
                    <option value="">Seleccionar servicio</option>
                    <?php foreach ($services as $service): ?>
                        <option value="<?php echo $service['id']; ?>"
                                <?php echo ($formData['id_prestacion'] ?? '') == $service['id'] ? 'selected' : ''; ?>>
                            <?php echo htmlspecialchars($service['nombre']); ?>
                            <?php if (!empty($service['tipo_prestacion_nombre'])): ?>
                                (<?php echo htmlspecialchars($service['tipo_prestacion_nombre']); ?>)
                            <?php endif; ?>
                        </option>
                    <?php endforeach; ?>
                </select>
                <?php if (isset($formErrors['id_prestacion'])): ?>
                    <div class="invalid-feedback"><?php echo $formErrors['id_prestacion']; ?></div>
                <?php endif; ?>
            </div>
            ...
            <div class="col-md-6">
                ...
                <small class="text-muted">Selecciona si el servicio se brinda a través de una empresa</small>
            </div>
        </div>
    </div>
    ...
    <div class="form-section">
        <div class="form-section-header">
            ...
            <h2 class="form-section-title">Período del Servicio</h2>
        </div>
        ...
        <div class="row g-3">
            ...
            <div class="col-12">
                <div class="form-check">
                    ...
                    <label class="form-check-label" for="es_recurrente">
                        Servicio Recurrente (sin fecha de finalización)
                    </label>
                </div>
            </div>
            ...
            <div class="col-12">
                ...
                <small class="text-muted">Días y horarios del servicio</small>
            </div>
        </div>
    </div>
        ...
        <div class="row g-3">
            ...
            <div class="col-12">
                ...
                <textarea class="form-control" id="observaciones" name="observaciones" rows="3"
                          placeholder="Información adicional sobre el servicio..."><?php echo htmlspecialchars($formData['observaciones'] ?? ''); ?></textarea>
            </div>
